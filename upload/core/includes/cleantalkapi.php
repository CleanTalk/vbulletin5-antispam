...
if ( !class_exists('CleantalkAPI') ) {
    ...
    private static $applied = 0;
            
    static function FormAddon($sType) {
	global $vbulletin;
	if($sType != 'autodetect' && $sType != 'register' && $sType != 'comment')
	    return '';
...
#            if (!session_id()) session_start();
            $myType = $sType;
            if ($myType == 'autodetect') {
                if ( isset($vbulletin->scriptpath) ) {
                    $s2 = substr($vbulletin->scriptpath, 0, 9);
                    if ($s2 == '/register') $myType = 'register';
                } else {
                    $myType = 'comment';
                }
            }
#	    $_SESSION['ct_submit_' . ($myType == 'register' ? 'register' : 'comment'). '_time'] = time();
	    $ct_check_value = md5(rand(0, 1000));
#	    $_SESSION['ct_check_key'] = $ct_check_value;
	    $ct_template_addon_body = '
<input type="hidden" name="ct_checkjs" id="ct_checkjs" value="0" />
<script type="text/javascript">
setTimeout("document.getElementById(\"ct_checkjs\").value = document.getElementById(\"ct_checkjs\").value.replace(\"0\", \"' . $ct_check_value .'\");",1000);
</script>
';
	    self::$applied = 1;
...
#            $checkjs = 0;
#        elseif(!isset($_POST['ct_checkjs']))
#            $checkjs = NULL;
#        elseif($_POST['ct_checkjs'] == $_SESSION['ct_check_key'])
#            $checkjs = 1;
#        else
#            $checkjs = 0;
	...
	if (!class_exists('Cleantalk')){
	    include_once(dirname(__FILE__).'/cleantalk/cleantalk.class.php');
	}
	...
	if (isset($_SERVER['HTTP_X_FORWARDED_FOR'])){
        ...
        $ct_request->agent = 'vbulletin5-11';
